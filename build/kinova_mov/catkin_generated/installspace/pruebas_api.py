...
class KinovaControl():
    ...
    def __init__(self):
        ...
        self.robot_args, unknown = self.parser.parse_known_args()
...
#            self.base = BaseClient(router)
    ...
    def send_joint_angles(self, base, joint_angles):
            command = Base_pb2.Action()
            command.name = "move_to_ik_result"
            command.application_data = ""

            for i, angle in enumerate(joint_angles.joint_angles):
                joint = command.reach_joint_angles.joint_angles.joint_angles.add()
                joint.joint_identifier = i
                joint.value = angle.value
            ...
            if finished:
                rospy.loginfo("IK-based joint movement executed.")
            else:
                rospy.logwarn("Timeout or failure in executing IK movement.")
    ...
    def calc_ik(self, base, position, orientation):
        try: 
            ...
            rospy.logerr(f"Unable to get current robot pose: {ex}")
            return False
        ...
        # Enviar al brazo
        self.send_joint_angles(base, computed_ik_angles)
...
def getmsg_callback(msg):
    ...
    rospy.loginfo("Position {} | Orientation {}".format(msg.pose.position, msg.pose.orientation))
...
def main():
    ...
    rospy.init_node("KinovaTesterController")
    rospy.loginfo("Initializing KinovaTesterController node with" + str(KC.robot_args))
    ...
    if str(KC.robot_args.ip) == "192.168.1.10":
        with utilities.DeviceConnection.createTcpConnection(KC.robot_args) as router:
            ...
            KC.base = base
            KC.example_move_to_home_position(KC.base)
